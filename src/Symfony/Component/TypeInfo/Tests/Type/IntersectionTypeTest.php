...
class IntersectionTypeTest extends TestCase
{
    ...
    public function testComposedTypesSatisfy()
    {
        $type = new IntersectionType(Type::object(\Iterator::class), Type::object(\Stringable::class));
        $this->assertTrue($type->composedTypesSatisfy(static fn (Type $t): bool => $t instanceof ObjectType));

        $type = new IntersectionType(Type::object(\Iterator::class), Type::object(\Stringable::class));
        $this->assertFalse($type->composedTypesSatisfy(static fn (ObjectType $t): bool => \Iterator::class === $t->getClassName()));
    }
