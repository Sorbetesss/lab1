...
class GenericTypeTest extends TestCase
{
    ...
    public function testWrappedTypeSatisfy()
    {
        $type = new GenericType(Type::builtin(TypeIdentifier::ARRAY), Type::bool());
        $this->assertTrue($type->wrappedTypeSatisfy(static fn (Type $t): bool => 'array' === (string) $t));

        $type = new GenericType(Type::builtin(TypeIdentifier::ITERABLE), Type::bool());
        $this->assertFalse($type->wrappedTypeSatisfy(static fn (Type $t): bool => 'array' === (string) $t));
    }
}
